...
def make_wizard(starting_position, starting_vel = vector(0, 0, 0)):
    """The lines below make a new "frame", which is a container with a
       local coordinate system.
       The arguments to make_alien allow for any initial starting position
       and initial starting velocity, with a default starting velocity
       ...
       alien's components:
    """
    alien_body = sphere(size = 1.0*vector(1, 1, 1), pos = vector(0, 0, 0), color = color.red)
    alien_eye1 = sphere(size = 0.3*vector(1, 1, 1), pos = .42*vector(.7, .5, .2), color = color.white)
    alien_eye2 = sphere(size = 0.3*vector(1, 1, 1), pos = .42*vector(.2, .5, .7), color = color.white)
    alien_hat = cone(pos = 0.42*vector(0, .9, -.2), axis = vector(.02, .2, -.02), size = vector(0.8, 0.7, 0.7), color = color.blue)
    alien_objects = [alien_body, alien_eye1, alien_eye2, alien_hat]  # make a list to "fuse" with a compound
...
wallC = box(pos = vector(10, 0, 0), axis = vector(0, 0, 1), size = vector(20, 1, .2), color = color.red) #red
wallD = box(pos = vector(0, 0, 10), axis = vector(1, 0, 0), size = vector(20, 1, .2), color = color.green) #green
...
wizard =  make_wizard(starting_position = vector(1, 5, 1))
wizard2 =  make_wizard(starting_position = vector(1, 1, -1), starting_vel = vector(-1, 0, 0))
...
while True:
    ...
    wizard2.pos = wizard2.pos + wizard2.vel*dt
